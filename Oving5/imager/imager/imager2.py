...
class Imager():
    ...
    _colors_ = ["green","red","pink","white","blue","orange","gold","violet","grey"]
    ...
    def n_concenate_vert(self,n):
        result = self
        for x in range(n - 1):
            result = self.concat_vert(im2=result)
        return result

    def vert_random_colors(self,n):
        result = self.colorize("yellow","black")
        for x in range(n -1):
            color_a = random.choice(Imager._colors_)
            color_b = random.choice(Imager._colors_)
            while (color_a == color_b):
                color_b = random.choice(Imager._colors_)
            pic = self.colorize(color_a,color_b)
            result = pic.concat_vert(im2=result)
        return result


    def horiz_random_colors(self,n):
        result = self.colorize("yellow", "black")
        for x in range(n - 1):
            color_a = random.choice(Imager._colors_)
            color_b = random.choice(Imager._colors_)
            while (color_a == color_b):
                color_b = random.choice(Imager._colors_)
            pic = self.colorize(color_a, color_b)
            result = pic.concat_horiz(im2=result)
        return result

    def n_concenate_horiz(self,n):
        result = self
        for x in range(n - 1):
            result = self.concat_horiz(im2=result)
        return result

    def xscale(self, xmax):
        yfactor = xmax / self.xmax
        return self.resize(xmax, self.ymax).scale(1, yfactor)

    def yscale(self, ymax):
        xfactor = ymax / self.ymax
        return self.resize(self.xmax, ymax).scale(xfactor, 1)
# _______ IMAGEOPS ________

    def colorize(self,black,white):
        im2 = self.gen_grayscale()
        im1 = im2.get_image()
        im1.mode = "L"
        im1 = ImageOps.colorize(im1,black,white)
        im1.mode = "RGB"
        return Imager(image=im1)

    def mirror(self):
        im1 = Imager(image=self.image)
        im2 = ImageOps.mirror(im1.image)
        return Imager(image=im2)


# _______ IMAGEDRAW ________

    def draw_text(self,text,color):
        draw = ImageDraw.Draw(self.get_image())
...
if __name__ == "__main__":
    p1 = "images/donaldduck.jpeg"
    p2 = "images/einstein.jpeg"
    p3 = "images/minions.jpeg"

    a = Imager(p3)
    ein = Imager(p2)
    donald = Imager(p1)
    ein = ein.resize(400,400)
    ein = ein.scale_colors(degree=5)
    donald = donald.resize(400,400)
    ein = ein.morph(donald,alpha=0.7)
    mange_a = a.horiz_random_colors(4)
    mange_a2 = a.horiz_random_colors(4)
    ein = ein.xscale(mange_a.xmax)
    donald = donald.xscale(ein.xmax)
    donald = donald.yscale(ein.ymax)
    result = mange_a.concat_vert(ein)
    result = result.concat_vert(mange_a2)
    mange_a3 = a.vert_random_colors(6)
    mange_a4 = a.vert_random_colors(6)
    result = result.yscale(mange_a3.ymax)
    result = mange_a3.concat_horiz(result)
    result = result.concat_horiz(mange_a4)
    result.draw_text("Eirik Dahlen","blue")
    result.display()
